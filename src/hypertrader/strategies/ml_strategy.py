...
# DataHandler import removed - not found in current structure
...
class EnhancedMLStrategy:
    """Enhanced Machine Learning based trading strategy with proper model handling."""
    
    def __init__(self, model_path: Optional[str] = None, retrain_interval: int = 1000):
        """Initialize ML strategy with model loading capability."""
        self.model = None
    ...
    def _initialize_model(self):
        ...
        try:
            if self.model_path:
                ...
                print("Initialized default ML model")
    ...
    def _preprocess_data(self, data: pd.DataFrame) -> pd.DataFrame:
        ...
        try:
            ...
            if len(features) < 10:
                # Return minimal features for safety
                return pd.DataFrame({'return': [0.0], 'rsi': [50.0]})
    ...
    def _retrain_model(self, data: pd.DataFrame):
        ...
        try:
            ...
            # Create target variable (next period returns > 0)
            target = (data['close'].shift(-1) > data['close']).loc[features.index]
            ...
            print(f"Model retrained with {len(features)} samples")
            
        except Exception as e:
            print(f"Model retraining failed: {e}")
    
    def generate_signal(self, data: pd.DataFrame) -> Signal:
        """Generate enhanced ML trading signal."""
        try:
            ...
            if len(volatility) > 0 and not pd.isna(volatility.iloc[-1]) and volatility.iloc[-1] > 0.08:
            ...
            try:
                ...
                if len(probabilities) >= 2:
                    confidence = max(probabilities)  # Use max probability as confidence
                    positive_prob = probabilities[1] if len(probabilities) > 1 else 0.5
                else:
                    confidence = 0.5
                    positive_prob = 0.5
            ...
            if positive_prob > 0.65:  # High confidence buy
                scaled_confidence = min(0.95, confidence * 1.2)
                return Signal('BUY', confidence=scaled_confidence)
            elif positive_prob > 0.55:  # Medium confidence buy
                scaled_confidence = min(0.75, confidence)
                return Signal('BUY', confidence=scaled_confidence)
            elif positive_prob < 0.35:  # High confidence sell
                scaled_confidence = min(0.95, confidence * 1.2)
                return Signal('SELL', confidence=scaled_confidence)
            elif positive_prob < 0.45:  # Medium confidence sell
                scaled_confidence = min(0.75, confidence)
                return Signal('SELL', confidence=scaled_confidence)
            else:
            ...
            print(f"ML signal generation failed: {e}")
            return Signal('HOLD', confidence=0.3)
